...
// Déterminer le level sélectionné via query param ou note
$selectedLevelId = $_GET['level_id'] ?? $note['level_id'];
    ...
    <div class="row">
        ...
        <div class="col-md-3 mb-3" style="max-height: 80vh; overflow-y: auto;">
            <?php foreach ($allLevels as $levelId => $levelName): ?>
                <a href="<?= route('note.edit', ['id' => $noteId]) . '?level_id=' . $levelId ?>" class="text-decoration-none mb-2 d-block">
        ...
        <div class="col-md-9">
            <div class="card shadow-sm">
                <div class="card-header bg-light py-3">
                    <h2 class="h5 mb-0">Édition d'une note</h2>
                </div>
                <div class="card-body">
                    <form action="<?= route('note.update', ['id' => $noteId]) ?>" method="post">
                        
                        <!-- Niveau (hidden) -->
                        <input type="hidden" name="level_id" value="<?= (int)$selectedLevelId ?>">

                        <div class="mb-3">
                            <?= select('student_id', $students, $note['student_id'], 'Étudiant') ?>
                        </div>

                        <div class="mb-3">
                            <?= select('year_id', $years, (string)$note['year_id'], 'Année scolaire') ?>
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-1"></i> Éditer la note
                            </button>
                        </div>
                    </form>
                </div>
