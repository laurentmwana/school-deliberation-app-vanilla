...
<div class="container py-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted">Créez et gérez les différentes classes de votre établissement</p>
        </div>
                        ...
                        <?php if (empty($levels)): ?>
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    ...
                                    <a href="<?= route('level.create') ?>" class="btn btn-primary mt-2">Créer votre première classe</a>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach($levels as $level): ?>
                            <tr>
                                <td class="ps-4 fw-bold"><?= $level['id'] ?></td>
                                <td>
                                  <?= $level['name'] ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="text-muted"><?= ago($level['created_at'] ?? new DateTime()) ?></div>
                                </td>
                                <td>
                                    <div class="d-flex justify-content-end gap-2 pe-3">
                                        <a href="<?= route('level.show', ['id' => $level['id']]) ?>" class="btn btn-sm btn-outline-primary d-flex align-items-center">
                                        ...
                                        <a href="<?= route('level.edit', ['id' => $level['id']]) ?>" class="btn btn-sm btn-outline-secondary d-flex align-items-center">
                                        ...
                                        <form method="post" action="<?= route('level.destroy', ['id' => $level['id']]) ?>" onsubmit="return confirm('Voulez-vous vraiment supprimer cette classe ? Cette action est irréversible.');">
