...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        // vérifier name
        if (findLevelIfExist('name', $name, $levelId)) {
            $errors['name'] = 'Ce nom est déjà utilisé par une autre classe.';
        }

        // vérifier alias
        if (findLevelIfExist('alias', $alias, $levelId)) {
    ...
    if (!empty($errors)) {
        ...
        saveOldInputs($_POST);
        redirect(route('level.edit', ['id' => $levelId]));
    } else {
        $state = updateLevel($levelId, compact('name', 'alias'));

        $state 
            ? flashMessage("success", "classe editée avec succès.")
            : flashMessage("danger", "Impossible d'editer la classe.");
