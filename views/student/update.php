...
if (!empty($_POST)) {
    ...
    // --- Patterns ---
    $namePattern      = '/^[\p{L} \-\'’]{2,50}$/u';
    $firstnamePattern = '/^[\p{L} \-\'’]{2,50}$/u';
    $genderValues     = ['M', 'F'];
    ...
    if (!empty($errors)) {
        ...
        saveOldInputs($_POST);
        redirect(route('student.edit', ['id' => $studentId]));
    } else {
        $state = updateStudent($studentId, compact('name', 'firstname', 'gender', 'level_id'));

        $state 
            ? flashMessage("success", "Étudiant modifié avec succès.")
            : flashMessage("danger", "Impossible de modifier l’étudiant.");
